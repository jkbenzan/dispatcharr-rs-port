...
pub async fn get_channels(
    ...
    let mut stream_obj_map: HashMap<i64, stream::Model> = HashMap::new();

    if !channel_ids.is_empty() {
        let backend = state.db.get_database_backend();

        // Batch channel groups
        let groups_stmt = sea_orm::sea_query::Query::select()
            .column(sea_orm::sea_query::Alias::new("channel_id"))
            .column(sea_orm::sea_query::Alias::new("channelgroup_id"))
            .from(sea_orm::sea_query::Alias::new(
                "dispatcharr_channels_channel_groups",
            ))
            .and_where(
                sea_orm::sea_query::Expr::col(sea_orm::sea_query::Alias::new("channel_id"))
                    .is_in(channel_ids.clone()),
            )
            .to_owned();
        let (sql, values) = groups_stmt.build_any(&*backend.get_query_builder());
        let groups = state
            .db
            ...
            .unwrap_or_default();
        for row in groups {
            if let (Ok(ch_id), Ok(gr_id)) = (
                row.try_get::<i64>("", "channel_id"),
                row.try_get::<i64>("", "channelgroup_id"),
            ) {
                groups_map.entry(ch_id).or_default().push(gr_id);
            }
        }

        // Batch channel profiles
        let profiles_stmt = sea_orm::sea_query::Query::select()
            .column(sea_orm::sea_query::Alias::new("channel_id"))
            .column(sea_orm::sea_query::Alias::new("channelprofile_id"))
            .from(sea_orm::sea_query::Alias::new(
                "dispatcharr_channels_channel_channel_profiles",
            ))
            .and_where(
                sea_orm::sea_query::Expr::col(sea_orm::sea_query::Alias::new("channel_id"))
                    .is_in(channel_ids.clone()),
            ...
            .to_owned();
        let (sql, values) = profiles_stmt.build_any(&*backend.get_query_builder());
        let profiles = state
            .db
            .query_all(Statement::from_sql_and_values(backend, sql, values))
            .await
            .unwrap_or_default();
        for row in profiles {
            if let (Ok(ch_id), Ok(pr_id)) = (
                row.try_get::<i64>("", "channel_id"),
                row.try_get::<i64>("", "channelprofile_id"),
            ) {
                profiles_map.entry(ch_id).or_default().push(pr_id);
            }
        }

        // Batch EPG sources
        let epg_stmt = sea_orm::sea_query::Query::select()
            .column(sea_orm::sea_query::Alias::new("channel_id"))
            .column(sea_orm::sea_query::Alias::new("epgsource_id"))
            .from(sea_orm::sea_query::Alias::new(
                "dispatcharr_channels_channel_epg_sources",
            ))
            .and_where(
                sea_orm::sea_query::Expr::col(sea_orm::sea_query::Alias::new("channel_id"))
                    .is_in(channel_ids.clone()),
            ...
            .to_owned();
        let (sql, values) = epg_stmt.build_any(&*backend.get_query_builder());
        let epgs = state
            .db
            .query_all(Statement::from_sql_and_values(backend, sql, values))
            .await
            .unwrap_or_default();
        for row in epgs {
            if let (Ok(ch_id), Ok(epg_id)) = (
                row.try_get::<i64>("", "channel_id"),
                row.try_get::<i64>("", "epgsource_id"),
            ) {
                epg_map.entry(ch_id).or_default().push(epg_id);
            }
        }

        // Batch stream links
        let streams_stmt = sea_orm::sea_query::Query::select()
            .column(sea_orm::sea_query::Alias::new("channel_id"))
            .column(sea_orm::sea_query::Alias::new("stream_id"))
            .from(sea_orm::sea_query::Alias::new(
            ...
            .to_owned();
        let (sql, values) = streams_stmt.build_any(&*backend.get_query_builder());
        ...
        for row in stream_links {
            if let (Ok(ch_id), Ok(st_id)) = (
                row.try_get::<i64>("", "channel_id"),
                row.try_get::<i64>("", "stream_id"),
            ) {
                stream_links_map.entry(ch_id).or_default().push(st_id);
                stream_ids.push(st_id);
            }
        }
        ...
        if !stream_ids.is_empty() {
            let streams = stream::Entity::find()
                ...
                .unwrap_or_default();
            for st in streams {
                stream_obj_map.insert(st.id, st);
            }
        }
    }
    ...
    for ch in channels {
        ...
        let empty_vec = vec![];
        let group_ids = groups_map.get(&ch.id).unwrap_or(&empty_vec);
        ch_json["channel_groups"] = json!(group_ids);

        let profile_ids = profiles_map.get(&ch.id).unwrap_or(&empty_vec);
        ch_json["channel_profiles"] = json!(profile_ids);

        let epg_ids = epg_map.get(&ch.id).unwrap_or(&empty_vec);
        ch_json["epg_sources"] = json!(epg_ids);

        let mut streams_arr = vec![];
        if let Some(s_ids) = stream_links_map.get(&ch.id) {
            for s_id in s_ids {
                if let Some(s_obj) = stream_obj_map.get(s_id) {
                    streams_arr.push(s_obj.clone());
                }
            }
        }
    ...
    let next_page = if has_next {
        Some(format!("/api/channels/channels/?page={}", page + 1))
    } else {
        None
    };
...
pub async fn get_streams(
    ...
    let paginated = filtered_streams
        .into_iter()
    ...
    let has_next = (offset + page_size) < count;
    let next_page = if has_next {
        Some(format!("/api/channels/streams/?page={}", page + 1))
    } else {
        None
    };
    let prev_page = if page > 1 {
...
pub async fn get_channel_ids(State(state): State<Arc<AppState>>) -> Json<Value> {
...
async fn get_channel_groups_for_account(
    account_id: i64,
...
async fn create_default_profile(
    ...
    let _ = m3u_account_profile::Entity::insert(new_profile)
        .exec(db)
        .await;
}

fn extract_custom_props_to_root(
    acc: &crate::entities::m3u_account::Model,
    acc_json: &mut serde_json::Value,
) {
...
fn apply_custom_props_from_payload(
    payload: &serde_json::Value,
    active: &mut crate::entities::m3u_account::ActiveModel,
    existing_props: Option<&serde_json::Value>,
) -> Option<bool> {
                ...
                if !url.is_empty() || file_path.is_some() {
                    ...
                    tokio::spawn(async move {
                        let error_msg = if is_xc {
                            ...
                            match crate::m3u::fetch_and_parse_m3u(
                                &db_clone,
                                &parse_url,
                                account_id,
                                true,
                        ...
                        if let Some(msg) = error_msg {
                                ...
                                .await
                            {
                                ...
                                active.last_message =
                                    sea_orm::Set(Some(msg.chars().take(255).collect()));
...
pub async fn get_m3u_account(
    ...
    match m3u_account::Entity::find_by_id(account_id)
        .one(&state.db)
        .await
    {
        Ok(Some(acc)) => {
...
pub async fn get_epg_source(
    ...
    match epg_source::Entity::find_by_id(source_id)
        .one(&state.db)
        .await
    {
...
pub async fn refresh_m3u_account(
    ...
    let account = match m3u_account::Entity::find_by_id(account_id)
        .one(&state.db)
        .await
    {
        Ok(Some(acc)) => acc,
    ...
    if !url.is_empty() {
        ...
        (
            StatusCode::ACCEPTED,
            Json(json!({"status": "M3U refresh task started"})),
        )
    } else {
        (
...
pub async fn refresh_vod(
    ...
    if account.account_type != "XC" {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "VOD refresh is only available for XtreamCodes accounts"})),
        );
    }
    ...
    if !vod_enabled {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "VOD is not enabled for this account"})),
        );
    }
    ...
    (
        StatusCode::ACCEPTED,
        Json(json!({"message": format!("VOD refresh initiated for account {}", account.name)})),
    )
}
...
pub async fn update_m3u_account(
    ...
    let acc = match m3u_account::Entity::find_by_id(account_id)
        .one(&state.db)
        .await
    {
        Ok(Some(a)) => a,
...
pub async fn create_m3u_profile(
    ...
    let active = m3u_account_profile::ActiveModel {
        ...
        is_default: sea_orm::Set(
            payload
                .get("is_default")
                .and_then(|v| v.as_bool())
                .unwrap_or(false),
        ),
        max_streams: sea_orm::Set(
            payload
                .get("max_streams")
                .and_then(|v| v.as_i64())
                .unwrap_or(1) as i32,
        ),
        is_active: sea_orm::Set(
            payload
                .get("is_active")
                .and_then(|v| v.as_bool())
                .unwrap_or(true),
        ),
        search_pattern: sea_orm::Set(
            payload
                .get("search_pattern")
                .and_then(|v| v.as_str())
                .unwrap_or("^(.*)$")
    ...
    if let Ok(inserted) = m3u_account_profile::Entity::insert(active)
        ...
        .await
    {
        ...
        (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "Failed to create profile"})),
        )
    }
}
...
pub async fn update_m3u_profile(
    ...
    use crate::entities::m3u_account_profile;
    if let Ok(Some(profile)) = m3u_account_profile::Entity::find_by_id(profile_id)
        .one(&state.db)
        .await
    {
...
pub async fn delete_m3u_profile(
    ...
    use crate::entities::m3u_account_profile;
    let _ = m3u_account_profile::Entity::delete_by_id(profile_id)
        .exec(&state.db)
        .await;
...
pub async fn create_m3u_filter(
    ...
    if let Ok(inserted) = m3u_filter::Entity::insert(active)
        .exec_with_returning(&state.db)
        .await
    {
        (StatusCode::CREATED, Json(json!(inserted)))
        ...
        (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "Failed to create filter"})),
        )
    }
}
...
pub async fn update_m3u_filter(
    ...
    use crate::entities::m3u_filter;
    if let Ok(Some(filter)) = m3u_filter::Entity::find_by_id(filter_id)
        .one(&state.db)
        .await
    {
    ...
    (
        StatusCode::BAD_REQUEST,
        Json(json!({"error": "Failed to update filter"})),
    )
}
...
pub async fn delete_m3u_filter(
    ...
    use crate::entities::m3u_filter;
    let _ = m3u_filter::Entity::delete_by_id(filter_id)
        .exec(&state.db)
        .await;
    (StatusCode::NO_CONTENT, Json(json!({})))
}
...
// --- Server Groups ---
pub async fn get_server_groups(State(state): State<Arc<AppState>>) -> impl IntoResponse {
...
pub async fn create_server_group(
    ...
    if let Ok(inserted) = server_group::Entity::insert(active)
        .exec_with_returning(&state.db)
        .await
    {
        (StatusCode::CREATED, Json(json!(inserted)))
        ...
        (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "Failed to create server group"})),
        )
    }
}
...
pub async fn update_server_group(
    ...
    use crate::entities::server_group;
    if let Ok(Some(group)) = server_group::Entity::find_by_id(group_id)
        .one(&state.db)
        .await
    {
    ...
    (
        StatusCode::BAD_REQUEST,
        Json(json!({"error": "Failed to update server group"})),
    )
}
...
pub async fn delete_server_group(
    ...
    use crate::entities::server_group;
    let _ = server_group::Entity::delete_by_id(group_id)
        .exec(&state.db)
        .await;
    (StatusCode::NO_CONTENT, Json(json!({})))
}
...
pub async fn refresh_m3u_all(State(state): State<Arc<AppState>>) -> impl IntoResponse {
        ...
        if !url.is_empty() {
            ...
            tokio::spawn(async move {
                ...
                if let Some(msg) = error_msg {
                    eprintln!("{}", msg);
                    if let Ok(Some(acc)) =
                        crate::entities::m3u_account::Entity::find_by_id(account_id)
                            .one(&db_clone)
                            .await
                    {
    ...
    (
        StatusCode::ACCEPTED,
        Json(json!({"success": true, "message": "M3U refresh initiated."})),
    )
}
...
pub async fn refresh_m3u_account_info(
    ...
    (
        StatusCode::ACCEPTED,
        Json(
            json!({"success": true, "message": format!("Account info refresh initiated for profile {}", profile_id)}),
        ),
    )
}
...
mod tests {
    ...
    async fn test_get_core_settings() {
        ...
        let get_val =
            |key: &str| settings.iter().find(|s| s["key"] == key).unwrap()["value"].clone();
