...
pub async fn list_users(
    ...
    let all_groups = accounts_user_groups::Entity::find()
    ...
    for g in all_groups {
        user_groups_map.entry(g.user_id).or_default().push(g.group_id);
    }
...
pub async fn list_groups(
    ...
    let all_perms = auth_group_permissions::Entity::find()
        .filter(auth_group_permissions::Column::GroupId.is_in(group_ids))
        .all(&state.db)
        .await
        .unwrap_or_default();

    let mut group_perms_map: std::collections::HashMap<i32, Vec<i32>> = std::collections::HashMap::new();
    for p in all_perms {
        group_perms_map.entry(p.group_id).or_default().push(p.permission_id);
    }

    let mut result = Vec::new();
    for g in groups {
        let perms = group_perms_map.get(&g.id).cloned().unwrap_or_default();
