<script lang="ts">
	...
	import { ChevronDown, ChevronRight, Play, Database, Layers, Eye, EyeOff } from 'lucide-svelte';
	import { onMount } from 'svelte';

	// --- Types ---
	interface Stream { id: number; name: string; url: string | null; channel_group: any; m3u_account_id: number; }
	interface M3UGroupView { name: string; expanded: boolean; streams: Stream[]; }
	interface M3UView { id: number; name: string; expanded: boolean; groups: M3UGroupView[]; total_streams: number; }
	...
	function resolveGroupName(channelGroup: any): string {
		...
		if (typeof channelGroup === 'number') return channelGroupNames.get(channelGroup) || `Group #${channelGroup}`;
	...
	function buildTree(m3us: any[], streams: any[]) {
		...
		streams.forEach(s => {
			...
			if (!grp.has(groupName)) grp.set(groupName, []);
			grp.get(groupName)!.push(s);
		});
		m3uViews = m3us.map(m => {
	...
	function toggleSelect(streamId: number, e: MouseEvent) {
		// Shift-click: range select within visible list (simplified: just toggle)
...
<div class="streams-pane">
	...
	{#if selectedIds.size > 0}
		<div class="selection-bar">
			<span>{selectedIds.size} stream{selectedIds.size > 1 ? 's' : ''} selected</span>
					...
					<div class="groups-list">
						{#each m3u.groups as group (group.name)}
							...
							<div class="group-row">
								...
								<!-- Select all in group -->
								{#if group.expanded}
									<button class="select-all-btn" title="Select/deselect all in group" onclick={() => toggleGroupSelect(group.streams)}>
...
.selection-bar {
	...
	padding: 6px 16px; background: rgba(237,28,36,0.1); border-bottom: 1px solid rgba(237,28,36,0.25);
	font-size: 12px; color: var(--accent);
	button { background: transparent; border: none; color: var(--text-dim); cursor: pointer; font-size: 11px; &:hover { color: var(--text-bright); } }
}
