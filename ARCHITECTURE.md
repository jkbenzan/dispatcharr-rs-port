...
- **Stream Checker & Maintenance**:
    ...
    - **Sorting Rules UI**: The Stream Checker page (`/stream-checker`) uses a tabbed interface (Testing | Sorting Rules). The Sorting Rules tab provides an inline CRUD table with property/operator dropdowns, property-specific value pickers, priority sequencing, score modifier input, and two-click delete confirmation. The table supports both inline editing of existing rules and inline creation of new rules. Backend rule saves trim and validate payloads, reject unsupported properties/operators with HTTP 400, and assign ids explicitly so rule creation does not fail when table grants exist but the backing PostgreSQL sequence lacks USAGE permissions.
- **Channel Matching**:
...
- **M3U Provider Management**:
    ...
    - Channel categories use a robust, data-backed classifier that parses structured prefix formats (e.g., `US |`, `|UK|`, `┃DE┃`) and resolves full ISO-3166-1 country names, aliases, and ambiguous codes. It also accurately classifies non-country clusters such as Sports, PPV, Live Events, 24/7, and Movies into regional/content buckets. The UI displays relevant emojis/flags on category cards and exposes a stable, compact, alphabetized detected-category filter while preserving plain text search for unclassified category sets.
    ...
    - **VOD Category Selection UI**: The provider modal provides dedicated "VOD Movies" and "VOD Series" tabs for granular category selection. These tabs use the same robust classification engine as Live TV categories to identify countries and streaming services (Netflix, Disney+, etc.), providing flags and human-readable metadata.
    - **Selection Reactivity**: Category lists use `$derived` runes for real-time filtering by search and country. Switching tabs automatically resets search and filter states to ensure a clean UI context.
    - **Backend Parity**: Category-to-account mappings rely on explicit `id` and `enabled` fields provided by the `vod.rs` and `api.rs` handlers, ensuring accurate selection state and provider-specific visibility.
    - **Bulk Actions**: VOD tabs support "Select visible" and "Deselect visible" actions, and the entire row acts as a toggle for improved accessibility and speed.
    - **Category Discovery**: VOD categories are discovered during the initial provider sync. Enabling a category persists the relation and triggers a refresh to populate the local library.
