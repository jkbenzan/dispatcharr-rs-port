...
- **Channel Manager** (`/channels`):
    ...
    - **Category vs Channel Group**: Streams carry a `group_title` field from the M3U provider's `#EXTINF group-title` attribute — this is the **Category** (provider-side grouping). The `channel_group` FK on the stream table points to the curated **Channel Group** (user-created organizational containers in Channel Manager). The StreamsPane groups streams by Category (`group_title`), while ChannelsPane groups channels by Channel Group. The backend surfaces `group_title` from `custom_properties` in the stream JSON response.
