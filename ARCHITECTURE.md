...
- **M3U Provider Management**:
    ...
    - Channel categories use lightweight country/region detection when provider group names look country-based. The UI shows detected flags on category cards and exposes a searchable detected-country filter while preserving plain text search for ambiguous or mixed category sets.
